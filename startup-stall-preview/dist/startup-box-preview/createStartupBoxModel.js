...
function makeLidMessageTexture() {
  ...
  ctx.font = '700 126px "Microsoft YaHei", sans-serif';
  ctx.fillText('摆摊', 66, 158);
  ctx.font = '700 78px "Microsoft YaHei", sans-serif';
  ctx.fillText('是我做过最快的', 66, 344);
  ctx.font = '700 126px Arial, sans-serif';
  ctx.fillText('mvp!', 66, 548);
  const texture = new THREE.CanvasTexture(canvas);
  texture.colorSpace = THREE.SRGBColorSpace;
  return texture;
}
...
export function createStartupBoxModel() {
  ...
  clothBundle.userData = { interactive: 'cloth-bundle', label: '黄色格纹布包' };
